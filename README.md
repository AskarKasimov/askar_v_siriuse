...
Заменив поле `format` с `JSON` на `XML`, можно с лёгкостью написать `XML`-инъекцию в поле *data*, позволяющую прочитать файл с сервера:
...
FLAG = [277, 92, 775, 480, 160, 92, 31, 586, 277, 801, 355, 489, 801, 31, 62, 926, 725, 489, 160, 92, 31, 586, 277, 801, 355, 489, 1281, 62, 801, 489, 1175, 277, 453, 489, 453, 348, 725, 31, 348, 864, 864, 348, 453, 489, 737, 288, 453, 489, 889, 804, 96, 489, 801, 721, 775, 926, 1281, 631]

if __name__ == "__main__":
  dihedral = DihedralCrypto(1337)
  answer = dihedral.hash([val for val in range(1, 256)])
  for ch in FLAG:
    print(chr(answer.index(ch) + 1), end='')
```
